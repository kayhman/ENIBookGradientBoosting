...
strong_predictor = lambda x: T0.predict(x) + T1.predict(x) + T2.predict(x)

squared_error = lambda y_true, y_pred : (y_true - y_pred)**2
x_train = [1, 2, 3, 4]
y_train = [1, 2, 3, 4]

T0_mse = np.mean([squared_error(x, T0.predict({'x': x})) for x, y in zip(x_train, y_train)])
T1_mse = np.mean([squared_error(x, T1.predict({'x': x})) for x, y in zip(x_train, y_train)])
T2_mse = np.mean([squared_error(x, T2.predict({'x': x})) for x, y in zip(x_train, y_train)])

strong_mse = np.mean([squared_error(x, strong_predictor({'x': x})) for x, y in zip(x_train, y_train)])

# train_ensemble_regularisation.py
    ...
    def predict(self, data):
        ...
        for _, row in data.iterrows():
            ...
            for tree_idx, root in enumerate(self.roots):
                ...
                # don't count base_score twice for root node
                if child != root:
                    pred += child.value
            preds.append(pred)
...
tree = DecisionEnsemble(squared_error, 1, 6)
tree.fit(x_train, y_train['Y'])
tree.dump('is_positive.rst')
pred = tree.predict(pd.DataFrame({'A': [-3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0]}))
...
tree = DecisionEnsemble(squared_error, 1, 6)
tree.fit(x_train, y_train['Y'])
tree.dump('is_positive_noisy.rst')
pred = tree.predict(pd.DataFrame({'A': [-3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0]}))
print(pred) #-> [-0.9040482  -0.9865432  -0.99615836  1.0572047   0.8786878   0.7685069  0.9598383 ]


tree = DecisionEnsemble(squared_error, 1, 6, gamma=1)
tree.fit(x_train, y_train['Y'])
tree.dump('is_positive_noisy_regularized.rst')
pred = tree.predict(pd.DataFrame({'A': [-3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0]}))
print(pred) #-> [-1. -1. -1. 1. 1. 1. 1.]
print(y_train['Y'].tolist()) #-> [-0.96225   -0.96225   -0.96225    0.9160595  0.9160595  0.9160595 0.9160595]
